...
Assuming action, the conditional logic is processed before altering the \verb|aceSig| which is then gained to become \verb|ace2dist|.
...
Each \verb|ctrlGen| has a unique low pass filter that processes the signal that is then gained by -1 and applied to the associated governor via the \verb|tg_sig| value.
...
The following AGC settings are not entirely realistic, but useful in demonstrating the  functionality of the AGC model.
