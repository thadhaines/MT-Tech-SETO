...
if g.mac.n_sub~=0
  if flag == 0 % initialization
      ...
      g.sys.theta(busnum,1) = bus(busnum,3)*pi/180;  % terminal bus angle in radians
      ...
      v = g.mac.eterm(g.mac.mac_sub_idx,1).*exp(jay*g.sys.theta(busnum,1)); % voltage in real and imaginary parts in system reference frame 
      curr = curr.*exp(jay*(g.sys.theta(busnum,1)-phi));  % complex current in system reference frame 
...
%       end
  ...
  if flag == 1 % network interface computation 
     % vectorized computation
      ...
      g.mac.mac_ang(g.mac.mac_sub_idx,k) = g.mac.mac_ang(g.mac.mac_sub_idx,k)-g.sys.mach_ref(k)*ones(g.mac.n_sub,1); % wrt machine reference
...
%                               cos(g.mac.mac_ang(g.mac.mac_sub_idx,k)).*g.mac.psidpp; % real part of psi - NEW
      g.sys.psi_re(g.mac.mac_sub_idx,k) = g.mac.mac_spd(g.mac.mac_sub_idx,k).*(...
                              sin(g.mac.mac_ang(g.mac.mac_sub_idx,k)).*(-g.mac.psiqpp) + ...
                              cos(g.mac.mac_ang(g.mac.mac_sub_idx,k)).*g.mac.psidpp); % real part of psi - NEW2
%       g.sys.psi_im(g.mac.mac_sub_idx,k) = -cos(g.mac.mac_ang(g.mac.mac_sub_idx,k)).*(-g.mac.psiqpp) + ...
%                               sin(g.mac.mac_ang(g.mac.mac_sub_idx,k)).*g.mac.psidpp; % imag part of psi   - NEW
      g.sys.psi_im(g.mac.mac_sub_idx,k) = g.mac.mac_spd(g.mac.mac_sub_idx,k).*(...
                              -cos(g.mac.mac_ang(g.mac.mac_sub_idx,k)).*(-g.mac.psiqpp) + ...
                              sin(g.mac.mac_ang(g.mac.mac_sub_idx,k)).*g.mac.psidpp); % imag part of psi  - NEW2
  % end of interface
  end
...
%       Te = g.mac.pelect(g.mac.mac_sub_idx,k).*g.mac.mac_pot(g.mac.mac_sub_idx,1) + g.mac.mac_con(g.mac.mac_sub_idx,5).*mcurmag.*mcurmag;%gen base - NEW
      Te = g.mac.psidpp.*g.mac.curqg(g.mac.mac_sub_idx,k) - g.mac.psiqpp.*g.mac.curdg(g.mac.mac_sub_idx,k);%gen base NEW2
