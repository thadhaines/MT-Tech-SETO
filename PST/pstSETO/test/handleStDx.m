function dxVec = handleStDx(k, flag, slnVec)
...
%
% Syntax: dxVec = handleStDx(k, flag, slnVec)
%
...
%
%   Output:
%   dxVec - collection of system derivatives at data index k
%
%   History:
...
if flag == 0
    ...
    g.sys.fsdn = fsdn; % store cell array to g.sys (for lack of better place)
    g.sys.n_states = nStates;
    
    %fsdn % debug output
end

if flag == 1
    %% create derivative vector from col k of allocated derivative fields
    % input: index k to collect derivatives from
    % output: updated/returned dxVec... 
    % NOTE: if function only updates global variables, there is no need for a return value
    
    fprintf('DEBUG: Creating single vector of all g.x.dxstate(:,k)...\n')
    
    %k =8750/2; % for debug
    
    dxVec = zeros( g.sys.n_states,1); % intialize dxVec
    startN = 1; % used to split up vector according to number of states
    
    for ndx=1:size(g.sys.fsdn,1)
        
        % If the number of allocated states is larger than zero
        if g.sys.fsdn{ndx, 4} ~= 0
            % collect derivative loction
            f1 = g.sys.fsdn{ndx,1};
            f2 = g.sys.fsdn{ndx, 3};
            % place derivatives into dxVec using dynamic field names
            dxVec(startN:startN+g.sys.fsdn{ndx,4}-1) = g.(f1).(f2)(1:g.sys.fsdn{ndx,4}, k);
            startN = startN + g.sys.fsdn{ndx,4}; % increment starting dxVec index
        else
            % probably not executed...
            disp(['skpping... ', g.sys.fsdn(ndx,1), g.sys.fsdn(ndx,3) ] );
        end
    end
...
if flag == 2
    %% Write slnVec vector of values to states at index j
    % input = slnVec, index to write next states to (integration results)
    % output = none?
    % input should not be a global as it is internally generated in the ODE sovler
    fprintf('DEBUG: Performing g.x.state(:,j) = g.x.dxstate(:,k)...\n')
    slnVec = dxVec; % for debug
    ...
    for ndx=1:size(g.sys.fsdn,1)
        ...
        if g.sys.fsdn{ndx, 4} ~= 0
            ...
            f1 = g.sys.fsdn{ndx,1};
            f2 = g.sys.fsdn{ndx, 2};
            for i=1:g.sys.fsdn{ndx, 4}
                g.(f1).(f2)(i,j) = slnVec(startN+i-1); % write to state col j
            end
            startN = startN+g.sys.fsdn{ndx, 4};
        else
            % probably not executed...
            disp(['skpping... ', g.sys.fsdn(ndx,1), g.sys.fsdn(ndx,2) ] );
        end
    end
