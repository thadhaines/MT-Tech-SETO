...
if n_ivm ~=0
    if flag == 0 % initialization
        if i~=0
          ...
          Vt = g.mac.eterm(mac_ivm_idx,1).*exp(1j*g.sys.theta(busnum,1)); %Terminal voltage
          ...
          g.sys.psi_re(mac_ivm_idx,1) = real(E); %real-part of internal voltage used for network interface
          g.sys.psi_im(mac_ivm_idx,1) = imag(E); %imag-part of internal voltage used for network interface
    ...
    if flag == 1 % network interface computation 
        if i ~= 0
          ...
          g.sys.psi_re(mac_ivm_idx,k) = real(E); %real part of internal voltage
          g.sys.psi_im(mac_ivm_idx,k) = imag(E); %imag part of internal voltage
        end
    end
