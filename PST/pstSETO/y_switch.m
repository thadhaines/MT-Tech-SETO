...
if f_type < 4 || f_type==7
    ...
    bus_f(bus_idx(1),9) = -bf; % set fault bus B 
end
...
[Y_gf,Y_gncf,Y_ncgf,Y_ncf,V_rgf,V_rncf,bof] = red_ybus(bus_f,line_f);   % fault-on admittance matrix 
...
if f_type < 4
    ...
    bus_pf1(max_pf1b,1) = new_bus; % fault bus number
    bus_pf1(max_pf1b,2) = 1.0;
    bus_pf1(max_pf1b,3:7)=zeros(1,5);
    bus_pf1(max_pf1b,9) = bus_f(bus_idx(1),9); % B
    bus_pf1(max_pf1b,10) = 3; % bus type
