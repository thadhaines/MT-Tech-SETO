...
if ~isempty(ind_con)
    if flag == 0;
        ...
        if i == 0;
            ...
            ind_pot(:,7)=g.sys.basrad*ind_con(:,7)./ind_pot(:,4); %1/Tr
            rs=ind_con(:,4);
            xs=ind_con(:,5);
            Xm=ind_con(:,6);
            rr=ind_con(:,7);
            ...
            nmot=size(ind_con,1);
            motbus=g.sys.bus_int(ind_con(i,2));
            ind_pot(i,1)=g.sys.basmva/ind_con(i,3); %scaled mva base
            ...
            ind_pot(i,7)=g.sys.basrad*ind_con(i,7)/ind_pot(i,4); %1/Tr
            rs=ind_con(i,4);
            xs=ind_con(i,5);
            Xm=ind_con(i,6);
            rr=ind_con(i,7);
            ...
            if  ind_con(i,15)>1e-6 % motor running
                ...
                while err>=1e-8 && iter<30
                    iter=iter+1;
                    y=g.sys.basrad*slip_old/ind_pot(i,7); %s omega Tr
                    denom = 1+y*y;
                    ...
                    slip_new=ynew/g.sys.basrad*ind_pot(i,7);
                    err=abs(slip_old-slip_new);
                    slip_old=slip_new;
                end;
                if iter>=30
                ...
                slip(i,1)=slip_new;
                y=g.sys.basrad*slip(i,1)/ind_pot(i,7);
                denom= 1+y*y;
                zr=rs+y*ind_pot(i,6)/denom;
                zi=ind_pot(i,5) + ind_pot(i,6)/denom;
    ...
    if flag == 2
        ...
        if i == 0
            ...
            dvdp(i,k)=-(iqm*ind_pot(i,6)+vdp(i,k))*...
                ind_pot(i,7)+vqp(i,k)*slip(i,k)*g.sys.basrad;
            dvqp(i,k)=(idm*ind_pot(i,6)-vqp(i,k))*...
                ind_pot(i,7)-vdp(i,k)*slip(i,k)*g.sys.basrad;
