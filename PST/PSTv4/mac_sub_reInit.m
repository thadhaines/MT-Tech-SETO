...
%   08/26/20    20:54   Thad Haines     Fix of 'isempy' and use of k instead of 1 in flag 0 action
    ...
    if flag == 0 % initialization
        if i ~= 0
            ...
            if ~isempty(sub)
                ...
                %% modified to use input index
                % extract bus information
                g.mac.eterm(i,k) = bus(busnum,2);  % terminal bus voltage
                g.bus.theta(busnum,k) = bus(busnum,3)*pi/180;
                % terminal bus angle in radians
                g.mac.pelect(i,k) = bus(busnum,4)*g.mac.mac_con(i,22); % system base
                % electrical output power, active
                g.mac.qelect(i,k) = bus(busnum,5)*g.mac.mac_con(i,23);
                % electrical output power, reactive
                curr = sqrt(g.mac.pelect(i,k)^2+g.mac.qelect(i,k)^2) ...
                    /g.mac.eterm(i,k)*g.mac.mac_pot(i,1);  % current magnitude
                %on genenearor base
                phi = atan2(g.mac.qelect(i,k),g.mac.pelect(i,k));
                % power factor angle
                v = g.mac.eterm(i,k)*exp(1j*g.bus.theta(busnum,k));
                % complex voltage
                % in system reference frame
                curr = curr*exp(1j*(g.bus.theta(busnum,k)-phi)); % complex current
                % in system reference frame
                ei = v + (g.mac.mac_con(i,5)+1j*g.mac.mac_con(i,11))*curr;
                g.mac.mac_ang(i,k) = atan2(imag(ei),real(ei));
                % machine angle (delta)
                g.mac.mac_spd(i,k) = 1; % machine speed at steady state
                rot = 1j*exp(-1j*g.mac.mac_ang(i,k));  % system reference frame rotation
                curr = curr*rot;
                g.mac.curdg(i,k) = real(curr);
                g.mac.curqg(i,k) = imag(curr);% current in Park's frame
                g.mac.curd(i,k) = real(curr)/g.mac.mac_pot(i,1);% current on system base
                g.mac.curq(i,k) = imag(curr)/g.mac.mac_pot(i,1);
                mcurmag = abs(curr); % current magnitude on machine base
                g.mac.pmech(i,k) = g.mac.pelect(i,k)*g.mac.mac_pot(i,1) + g.mac.mac_con(i,5)*(mcurmag*mcurmag);
                %pmech = g.mac.pelect + losses on machine base
                v = v*rot;
                g.mac.ed(i,k) = real(v);
                g.mac.eq(i,k) = imag(v);
                eqra = g.mac.eq(i,k)+g.mac.mac_con(i,5)*g.mac.curqg(i,k);
                g.mac.psidpp = eqra + g.mac.mac_con(i,8)*g.mac.curdg(i,k);
                g.mac.psikd(i,k) = eqra + g.mac.mac_con(i,4)*g.mac.curdg(i,k);
                g.mac.eqprime(i,k) = eqra + g.mac.mac_con(i,7)*g.mac.curdg(i,k);
                edra = -g.mac.ed(i,k)-g.mac.mac_con(i,5)*g.mac.curdg(i,k);
                g.mac.psiqpp = edra + g.mac.mac_con(i,13)*g.mac.curqg(i,k);
                g.mac.psikq(i,k) = edra + g.mac.mac_con(i,4)*g.mac.curqg(i,k);
                g.mac.edprime(i,k) = edra + g.mac.mac_con(i,12)*g.mac.curqg(i,k);
                % compute saturation
                ...
                if g.mac.eqprime(i,k)<0.8
                    E_Isat=g.mac.eqprime(i,k);
                end
                g.mac.vex(i,k) = E_Isat + g.mac.mac_pot(i,6)*(g.mac.eqprime(i,k)-...
                    g.mac.psikd(i,k))+g.mac.mac_pot(i,7)*g.mac.curdg(i,k);
                g.mac.fldcur(i,k) = g.mac.vex(i,k);
