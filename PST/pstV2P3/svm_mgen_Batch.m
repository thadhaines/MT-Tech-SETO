...
if ~isempty(pwrmod_con)~=0
   f = pwrm_indx(bus); % identifies power modulation buses
else
   n_pwrmod = 0;
end
...
f = dc_cont(0,1,1,bus,0); % initialize the dc controls - sets up data for red_ybus
...
% check for infinite buses
if isempty(ibus_con)
   ref_gen = 'N';%input('Do you want to set gen 1 as reference, Y/N(N)','s');
   ...
   if ref_gen ~= 'N'
      ...
      if n_rlmod~=0;b_rlmod = p_ang*b_lmod;end
...
% disp(' c_ilmf is the output matrix for the line current magnitude at the from buses')
% disp(' c_ilmt is the output matrix for the line current magnitude at the to buses')
% disp(' c_ilrf is the output matrix for the real part of the line current at the from buses')
% disp(' c_ilrt is the output matrix for the real part of the line current at the to buses')
% disp(' c_ilif is the output matrix for the imaginary part of the line current at the from buses')
% disp(' c_ilit is the output matrix for the imaginary part of the line current at the to buses')
% 
% 
% 
...
% disp(' b_tcsc is the input matrix corresponding to the tcsc input')
...
% disp(' b_pwrmod_p is the input matrix corresponding to the pwrmod P input signal')
% disp(' b_pwrmod_q is the input matrix corresponding to the pwrmod Q input signal')
% 
% disp(' ')
...
% disp(' ')
% disp(' the state variable numbers for the kth device')
...
%    disp('     5 to see paticipation factors associated with the modes having damping ratio less than 0.05')
...
%             title(['participation factors of mode ',mode_num,'; damping ',str_damp,'; frequency ',str_freq])
