...
if nargout <= 2
    ...
    if nload~=0
        ...
        Y_b1 = Y_b(:,1:nload);
        Y_b2 = Y_b(:,nload+1:nbus);
        Y_c1 = Y_c(1:nload,:);
        Y_c2 = Y_c(nload+1:nbus,:);
        Y_d1 = Y_d(1:nload,:);
        Y_d2 = Y_d(nload+1:nbus,:);
        Y_d11 = Y_d1(:,1:nload);
        Y_d12 = Y_d1(:,nload+1:nbus);
        Y_d21 = Y_d2(:,1:nload);
        Y_d22 = Y_d2(:,nload+1:nbus);
        %
        %yinv = inv(Y_d22);
        %rec_V1 = -yinv*Y_c2; rec_V2 = -yinv*Y_d21;
        rec_V2 = -Y_d22\[Y_c2 Y_d21];
        rec_V1 = rec_V2(:,1:n_tot);
        rec_V2 = rec_V2(:,n_tot+1:n_tot+nload);
        ...
        if n_conv~=0
            ...
            x_dc(r_idx) = dcc_pot(:,2);
            x_dc(i_idx) = dcc_pot(:,4);
