...
% All Rights Reserved
% Added code for pwrmod, D. Trudnows, 2015
% step 3a: network solution
flag = 1;
%generators
f = mac_ib(0,2,bus,flag);
f = mac_em(0,2,bus,flag);
f = mac_tra(0,2,bus,flag);
f = mac_sub(0,2,bus,flag);
f = mac_ind(0,2,bus,flag); 
f = mac_igen(0,2,bus,flag);
...
f=pss(0,2,bus,flag);
% exciters
f = smpexc(0,2,bus,flag);
f = smppi(0,2,bus,flag);
f = exc_dc12(0,2,bus,flag);
f = exc_st3(0,2,bus,flag);
% turbine/governor
f = tg(0,2,bus,flag);
f = tg_hydro(0,2,bus,flag);
...
if n_pwrmod~=0 
   f = pwrmod_p(0,2,bus,flag);
   f = pwrmod_q(0,2,bus,flag);
end
if n_conv ~=0
   f = dc_cont(0,2,2,bus,flag);
   f = dc_line(0,2,2,bus,flag);
end
