...
if n_st3~=0
  ...
  %check for either mac_sub or mac_tra models
	...
	error('I_N zero or negative: check data input')
      ...
      vep = eterm(n,1).*exp(jay*theta(n_bus,1)).*(exc_pot(st3_idx,1) + jay*exc_pot(st3_idx,2)); % corrected theta index to n_bus from n per Ryan - thad 07/01/20
  ...
  if flag == 1 % network interface computation
    ...
    %vector calculation
      ...
      E_Isat(n) = max(eqprime(n,k),E_Isat(n)); % select higher
                                         % voltage
   % added . to .* multiplies -thad 06/23/20                                      
