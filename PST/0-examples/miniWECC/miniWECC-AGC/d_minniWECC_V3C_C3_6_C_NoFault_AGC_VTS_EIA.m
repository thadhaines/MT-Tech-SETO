...
% Addition of EIA areas, AGC, and optional VTS by Thad Haines 09/30/20
...
if ~exist('enableVTS','var')
    enableVTS = 1; % Default action to allow VTS
end
...
% col 1 bus number
% col 2 area number
%NOTE: based on estimation of EAI regions

a2Bus = [56, 57, 67, 61, 63, 64, 66, 92, 93, 95, ...
    97, 98, 58, 59, 65, 62, 60];

a3Bus = [ 42, 43, 115, 46, 49, 52, 54, 91, 116, 99, ...
    114, 44, 40, 38, 90, 113, 37, 39, 41, 47, ...
    45, 48, 51, 53, 50, 55, 112];

area_def = zeros(size(bus,1),2);

for n = 1:size(bus,1)
    
    if ~isempty(intersect(n, a2Bus))
        areaN = 2;
    elseif ~isempty(intersect(n, a3Bus))
        areaN = 3;
    else
        areaN = 1;
    end
...
%  5.0   86   87    0    0    0    0.001; %apply fault (three phase)
%  5.095  0    0    0    0    0    0.005; %clear near end of fault
%  5.1    0    0    0    0    0    0.005; %clear far end of fault
%  5.5    0    0    0    0    0    0.005; %increas time step
%  50.0   0    0    0    0    0    0]; %end simulation

%PV drop (gen 26)
% sw_con = [...
%  0      0    0    0    0    0    0.005;%sets intitial time step
%  5.0   60   61    0    0    4    0.001; %apply fault (loss of line no fault)
%  5.5    0    0    0    0    0    0.005; %increas time step
%  100.0   0    0    0    0    0    0]; %end simulation

%Colstrip drop (gen 14)
% sw_con = [...
%  0      0    0    0    0    0    0.005;%sets intitial time step
%  5.0   32   33    0    0    4    0.001; %apply fault (loss of line no fault)
%  5.5    0    0    0    0    0    0.005; %increas time step
%  50.0   0    0    0    0    0    0]; %end simulation
...
%  7.0    6  106    0    0    0    1/1200; %apply fault (three phase)
%  7.095  0    0    0    0    0    1/1200; %clear near end of fault
%  7.1    0    0    0    0    0    1/1200; %clear far end of fault
%  7.3    0    0    0    0    0    1/120; %increas time step
%  20.0   0    0    0    0    0    0]; %end simulation

% %No Fault
% sw_con = [...
%  0        0    0    0    0    0    1/120;%sets intitial time step
%  1    6  106    0    0    6    1/120; % do nothing
%  25    0    0    0    0    0    1/120; %increas time step
%  55    0    0    0    0    0    1/120; %increas time step
%  600    0    0    0    0    0    0]; %end simulation

%No Fault - AGC 10 Minute
sw_con = [...
    0        0    0    0    0    0    1/120;% pre-event
    %1    6  106    0    0    6    1/120; % P step event
    5    6  106    0    0    6    1/120; % trip event
    8    0  0    0    0    0    1/120; % trip event
    40    0    0    0    0    0    1/120; % first AGC ctrl signals
    % 160    0    0    0    0    0    1/120; % following AGC
