...
if kT<2
    ...
    if abs(t(kT)-5)<1e-5
        ...
        mac_trip_states(3,:) = [3; g.sys.t(kT)]; %keep track of when things trip
        disp(['MAC_TRIP_LOGIC:  Tripping gen 3 at t = ' num2str(g.sys.t(kT))])
        for n=0:1
    ...
    if abs(t(kT)-15.0)<1e-5 %
        disp(['MAC_TRIP_LOGIC:  "Un-Tripping" gen 3 at t = ' num2str(g.sys.t(kT))])
        tripOut(3) = false;
    ...
    %% re-init gov, ramp R in
    if abs(g.sys.t(kT)-20) < 1e-5
        disp(['MAC_TRIP_LOGIC:  reinit gov, start ramping R in at t = ', num2str(g.sys.t(kT))])
        reInitGov(3,kT)
    end
    ...
    if abs(t(kT)-80.0)<1e-5 % ramp exciter reference voltage
        disp(['MAC_TRIP_LOGIC:  ramping exciter to original ref voltage at t = ', num2str(g.sys.t(kT))])
