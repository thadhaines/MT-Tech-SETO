...
if n_ivm~=0
    ...
    if (~iscell(Dini) || ~iscell(Eini)); error('Error in ivmmod_dyn, initial states must be cells'); end
    if (any(size(Dini)-[n_ivm 1]) || any(size(Eini)-[n_ivm 1])); error('Dimension error in ivmmod_dyn'); end
...
if n_pwrmod~=0
   ...
   f = pwrmod_q(0,1,bus,flag);
    [~,~,~,~,Pini,Qini] = pwrmod_dyn([],[],bus,t,0,0,n_pwrmod);
    if (~iscell(Pini) || ~iscell(Qini)); error('Error in pwrmod_dyn, P_statesIni and P_statesIni must be cells'); end
    if (any(size(Pini)-[n_pwrmod 1]) || any(size(Qini)-[n_pwrmod 1])); error('Dimension error in pwrmod_dyn'); end
    ...
    for index=1:n_pwrmod
        if ((size(Pini{index},2)~=1) || (size(Qini{index},2)~=1)); error('Dimension error in pwrmod_dyn'); end
        dpwrmod_p_sigst{index} = zeros(size(Pini{index},1),k); pwrmod_p_sigst{index} = Pini{index}*ones(1,k); 
        dpwrmod_q_sigst{index} = zeros(size(Qini{index},1),k); pwrmod_q_sigst{index} = Qini{index}*ones(1,k);
    end
...
while (kt<=ktmax)
   ...
   for k = k_start:k_end
      ...
      if n_pwrmod~=0
        ...
        if (~iscell(dp) || ~iscell(dq)); error('Error in pwrmod_dyn, dp and dq must be cells'); end
        if (any(size(dp)-[n_pwrmod 1]) || any(size(dq)-[n_pwrmod 1])); error('Dimension error in pwrmod_dyn'); end
        for index=1:n_pwrmod
            if ((size(dp{index},2)~=1) || (size(dq{index},2)~=1)); error('Dimension error in pwrmod_dyn'); end
            if size(dp{index},1)~=size(dpwrmod_p_sigst{index},1); error('Dimension error in pwrmod_dyn'); end
            if size(dq{index},1)~=size(dpwrmod_q_sigst{index},1); error('Dimension error in pwrmod_dyn'); end
        ...
        pwrmod_p_sig(:,k) = P;
        pwrmod_q_sig(:,k) = Q;
        f = pwrmod_p(0,k,bus_sim,flag);
        f = pwrmod_q(0,k,bus_sim,flag);
      ...
      if n_ivm>0
        ...
        if (length(d)~=n_ivm) || (length(e)~=n_ivm); error('Dimension error in ivmmod_dyn'); end
        ivmmod_d_sig(:,k) = d;
        ivmmod_e_sig(:,k) = e;
        f = mac_ivm(0,k,bus_sim,flag);
        [~,~,dd,de,~,~] = ivmmod_dyn(dst,est,bus,t,k,flag);
        if (~iscell(dd) || ~iscell(de)); error('Error in ivmmod_dyn, dd and de must be cells'); end
        if (any(size(dd)-[n_ivm 1]) || any(size(de)-[n_ivm 1])); error('Dimension error in ivmmod_dyn'); end
        for index=1:n_ivm
            if ((size(dd{index},2)~=1) || (size(de{index},2)~=1)); error('Dimension error in ivmmod_dyn'); end
            if size(dd{index},1)~=size(divmmod_d_sigst{index},1); error('Dimension error in ivmmod_dyn'); end
            if size(de{index},1)~=size(divmmod_e_sigst{index},1); error('Dimension error in ivmmod_dyn'); end
      ...
      if n_conv~=0
         ...
         f = dc_cont(0,k,k,bus_sim,2);
         f = dc_line(0,k,k,bus_sim,2);
      end
