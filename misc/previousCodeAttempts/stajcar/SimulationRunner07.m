...
[Bus,Branch,Gen,Gov] = funBusBranchGenGovInputs();
    ...
    switch Flag
        case 'subtransient'
            if InitializeSubTrans == 1
                % Calc RotAng, Efd, Psi's, and E's @ ii.
                [Gen] = funSynMacSub_GenTpJ(Gen,ii,Time.dtSubTrans,0);
                % Calc E @ ii.
                [Gen] = funSynMacSub_GenTpJ(Gen,ii,Time.dtSubTrans,1);
                % Inialize gov variables.
                for jj = 1:length(Gov)
                    Gov(jj).Pref(ii,1) = Gen(Gov(jj).GenNum).Pe(ii,1) + ...
                        abs(Gen(Gov(jj).GenNum).I(ii,1))^2 * Gen(Gov(jj).GenNum).R;
                end
                clear jj
                [Gov] = funGov_TGOV1(Gov,Gen,ii,Time.dtSubTrans,0);
                % Calc Pm @ ii.
                [Gov] = funGov_TGOV1(Gov,Gen,ii,Time.dtSubTrans,1);
                for jj = 1:length(Gov)
                    Gen(Gov(jj).GenNum).Pm(ii,1) = Gov(jj).Pm(ii,1); 
                end
                
                [Gen,Bus] = funInitialize4AdamsBashGenTpJ(Gen,Bus,ii);
                [Gov] = funInitialize4AdamsBashTGOV1(Gov,ii);
            ...
            % Calc Pm @ ii.
            for jj = 1:length(Gov)
                Gov(jj).Pref(ii,1) = Gov(jj).Pref(ii-1,1);
            end
            [Gov] = funGov_TGOV1(Gov,Gen,ii,Time.dtSubTrans,1);
            for jj = 1:length(Gov)
                Gen(Gov(jj).GenNum).Pm(ii,1) = Gov(jj).Pm(ii,1); 
            ...
            % Check if simulation is finished. Otherwise, update.
            if Time.SimEnd > (Time.SimTime(ii) + Time.dtSubTrans)
                ...
                % Integrate Gov variables x1 and x2.
                [Gov] = funGov_TGOV1(Gov,Gen,ii,Time.dtSubTrans,2);
